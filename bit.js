if (typeof window['$'] == 'undefined') {
    ...
    (function ($) {
        ...
        $.bitinit = function () {
             ...
             * @param callBack:获取图片资源的回调
             * @param option:H5 input的配置
             * @param option accept:接受的文件格式 image/*、gif、jpeg、png; video/*
             * @param option capture:打开系统默认的设备 camera--照相机；camcorder--摄像机
             * @created  wangxiaotian
             * */
            $.bitupload = function (callBack, option) {
                if (option === void 0) { option = {}; }
                if (!$.isNoValid(callBack)) {
                    if (typeof callBack == 'function') {
                        var defaultConfig = {
                            accept: 'image/*',
                            capture: ''
                        };
                        var uploadConfig = Object.assign(defaultConfig, option);
                        //创建 H5 input
                        var inputFileHtml = "<input id=\"js-bit-input-file-none\" type=\"file\" style=\"position:fixed !important;\" />";
                        $(inputFileHtml).appendTo('body');
                        console.log($('#js-bit-input-file-none'));
                        $('#js-bit-input-file-none').on('click tap', function () {
                        });
                        $('#js-bit-input-file-none').trig;
                    }
                    else {
                        console.error("$.bitimgUpload\u53C2\u6570callBack\uFF1A" + callBack + "\u5E94\u4E3AFunction");
                    }
                }
            },
            /**
